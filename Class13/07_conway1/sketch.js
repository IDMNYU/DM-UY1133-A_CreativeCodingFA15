...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      var p0 = img[whichimage].get(i-1, j-1)[0]>128; // upper left
      var p1 = img[whichimage].get(i, j-1)[0]>128; // upper mid
      var p2 = img[whichimage].get(i+1, j-1)[0]>128; // upper right
      var p3 = img[whichimage].get(i-1, j)[0]>128; // left
      var p4 = img[whichimage].get(i, j)[0]>128; // center pixel
      var p5 = img[whichimage].get(i+1, j)[0]>128; // right
      var p6 = img[whichimage].get(i-1, j+1)[0]>128; // lower left
      var p7 = img[whichimage].get(i, j+1)[0]>128; // lower mid
      var p8 = img[whichimage].get(i+1, j+1)[0]>128; // lower right
