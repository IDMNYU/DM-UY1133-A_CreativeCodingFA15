...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      var p0 = img[whichimage].get(i-1, j-1)[0]>0; // upper left
      var p1 = img[whichimage].get(i, j-1)[0]>0; // upper mid
      var p2 = img[whichimage].get(i+1, j-1)[0]>0; // upper right
      var p3 = img[whichimage].get(i-1, j)[0]>0; // left
      var p4 = img[whichimage].get(i, j)[0]>0; // center pixel
      var p5 = img[whichimage].get(i+1, j)[0]>0; // right
      var p6 = img[whichimage].get(i-1, j+1)[0]>0; // lower left
      var p7 = img[whichimage].get(i, j+1)[0]>0; // lower mid
      var p8 = img[whichimage].get(i+1, j+1)[0]>0; // lower right
